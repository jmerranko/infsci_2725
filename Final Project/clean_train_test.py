...
###The following was used from the kaggle scripts page to fix typos and spelling errors in search terms
###https://www.kaggle.com/steubk/home-depot-product-search-relevance/fixing-typos/comments
def str_stem(s):
    if isinstance(s, str) or isinstance(s, unicode):
        ...
        s = s.replace('moa','mower')        
        s = s.replace('deckover','deck over')
        s = s.replace('clouverleaf','cloverleaf')
        s = s.replace('grilll','grill')
...
#iterate through train and test search terms to fix errors and transform strings
with open('train_cleaned1.csv') as f:
    reader = csv.reader(f)
    out_file = open("train_cleaned.csv", "wb")
    writer=csv.writer(out_file)
    for row in reader:
        row[3]=row[3].encode("utf-8",'ignore')
        #row[3] = str_stem(row[3])
        row[3] = transformString(row[3])
        writer.writerow(row)
    out_file.close()

with open('test_cleaned1.csv') as f:
    reader = csv.reader(f)
    out_file = open("test_cleaned.csv", "wb")
    writer=csv.writer(out_file)
    for row in reader:
        row[3]=row[3].encode("utf-8",'ignore')
        #row[3] = str_stem(row[3])
